 ...
 * Loaded as a regular (non-module) script so every page can access
 * window.db, window.storage, window.firebaseApp from their own
 * <script type="module"> blocks AND from legacy inline scripts.
 *
 * Firebase SDK is imported via the compat (global) CDN build so
 * we don't need bundlers and it works in plain HTML files.
 */

/* ─── Firebase config ─────────────────────────────────────────────────── */
// NOTE: These keys are safe to expose in client-side code for a wedding site.
// Enable Firebase App Check or Firestore rules for production hardening.
...
/* ─── Bootstrap Firebase (compat SDK, loaded via CDN in each HTML) ─────── */
// Each page includes the compat CDN scripts before shared.js, so
// firebase.app() is already available here.
...
window.showToast = function(msg, type = 'pink') {
